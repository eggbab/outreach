...
## 5. (2차 라운드) 수집 채널 확장 & 법적 검토 — 2026-08-24 오후

### 5.1 채널 조사 결과
| 채널 | 방식 | 판단 |
|---|---|---|
| **카카오 로컬 API** | 공식 REST API, 일 10만 건 무료 | ✅ **구현** — 차단 리스크 0, 전화·주소·업종 제공. 스크래핑 차단 시의 보험 |
| 네이버 지역검색 오픈API | 공식, 쿼리당 최대 5건 | 보류 — 건수가 너무 적음 |
| 지방행정 인허가 데이터 | 공공데이터 (localdata.go.kr → 2026.4 data.go.kr 통합) | 로드맵 — 전화번호 포함 대량 시딩용. 월간 전체 파일 다운로드 배치로 GlobalProspect 시딩 |
| 소상공인 상가업소 데이터 | 공공데이터, 270만+ 업소 | 로드맵 — 상호·주소·업종 (연락처 없음, 보강용) |
| 인스타 해시태그 수집 | 비공식 | 반려 — 계정 벤 리스크 |

### 5.2 법적 검토 (판례 기준)
- **크롤링 자체**: 여기어때 대법원 무죄 확정(2022) — 로그인 없이 공개된 정보 수집은 정보통신망 침입·업무방해·저작권 위반 아님. 단 (a) 비공개/로그인 뒤 데이터 (b) 경쟁사 DB 통째 복제(잡코리아 v 사람인 민사 배상 사례) (c) 서버 부하 유발은 위험. → 우리는 공개 검색결과 소량 수집 + 공식 API 전환으로 안전권.
- **개인정보**: 공개된 사업자 연락처 수집은 대법원 로앤비 판례상 "공개 목적 범위 내" 처리 가능. 단 판매 시 원본 노출 금지 → discover 이메일 마스킹 유지가 법적으로도 올바른 설계.
- **발송**: 정보통신망법 §50 — (광고) 표기·전송자 정보·수신거부(1차 라운드 구현 완료)가 핵심 의무.
- **표시광고법**: 랜딩의 허위 후기(가공 인물)·근거 없는 수치("응답률 3배") 제거 → 사실 기반 카피로 교체.

### 5.3 이번 라운드 구현
- 카카오 수집기(`collector/kakao.py`) — 파이프라인 최우선 순위, 키 없으면 자동 스킵
- 이메일 MX 검증(`services/email_verify.py`) — 수집 시 email_valid 채움, 발송 시 invalid 스킵(무과금), GlobalProspect validity 초기값 반영
- 수집 파이프라인 중복 호출 버그 수정 + 소스별 수집 수 로깅 + 0건 시 사용자 안내
- GlobalProspect.region 실사용 시작 (카카오 주소 → 지역)
- 랜딩: "왜 Outreach인가" 신뢰 섹션 + 허위 후기/수치 제거
